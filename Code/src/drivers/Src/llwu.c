  ...
  //  LLWU_PE3 = temp;
   ...
   // temp = LLWU_PE4;
    if( pin_en & 0x1000)
    {
        ...
        printf(" LLWU configured pins PTD0/SPI0_PCS0/SCI2_RTS to be an LLWU wakeup source \n");
    //    LLWU_F2 |= LLWU_F2_WUF12_MASK;   // write one to clear the flag
    }
    if( pin_en & 0x2000)
    {
        temp |= LLWU_PE4_WUPE13(rise_fall);
        printf(" LLWU configured pins PTD2/SCI2_RX to be an LLWU wakeup source \n");
   //     LLWU_F2 |= LLWU_F2_WUF13_MASK;   // write one to clear the flag
    }
    if( pin_en & 0x4000)
    {
        temp |= LLWU_PE4_WUPE14(rise_fall);
        printf(" LLWU configured pins PTD4/SCI0_RTS/FTM0_CH4/EWM_IN is LLWU wakeup source \n");
    //    LLWU_F2 |= LLWU_F2_WUF14_MASK;   // write one to clear the flag
    }
    if( pin_en & 0x8000)
    {
        temp |= LLWU_PE4_WUPE15(rise_fall);
        printf(" LLWU configured pins PTD6/SCI0_RX/FTM0_CH6/FTM0_FLT0 is LLWU wakeup source \n");
...
//       LPTMR0_CSR = ( LPTMR_CSR_TEN_MASK | LPTMR_CSR_TIE_MASK | LPTMR_CSR_TCF_MASK  );
   ...
   if (LLWU_F3 & LLWU_F3_MWUF4_MASK) {
	     printf("\n [LLWU ISR] ****WUF3_MWUF3 IF TSI  *****\r\n"); 
       LLWU_F3 |= LLWU_F3_MWUF3_MASK;   // write one to clear the flag
   }
   if (LLWU_F3 & LLWU_F3_MWUF5_MASK) {
	     printf("\n [LLWU ISR] ****WUF3_MWUF3 IF RTC Alarm  *****\r\n"); 
       LLWU_F3 |= LLWU_F3_MWUF3_MASK;   // write one to clear the flag
   }
   if (LLWU_F3 & LLWU_F3_MWUF6_MASK) {
	     printf("\n [LLWU ISR] ****WUF3_MWUF3 IF DryIce(Tamper Detect)  *****\r\n"); 
       LLWU_F3 |= LLWU_F3_MWUF3_MASK;   // write one to clear the flag
   }
   if (LLWU_F3 & LLWU_F3_MWUF7_MASK) {
	     printf("\n [LLWU ISR] ****WUF3_MWUF3 IF RTC Seconds  *****\r\n"); 
       LLWU_F3 |= LLWU_F3_MWUF3_MASK;   // write one to clear the flag
   }
